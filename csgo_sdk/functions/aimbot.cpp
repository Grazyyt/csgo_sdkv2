...
bool LegitBot::IsEnabled(CUserCmd* pCmd) 
{
	...
	settings = g_Configurations.legitbot_items[pWeapon->m_Item().m_iItemDefinitionIndex()];
	if (!settings.enabled) 
		return false;
...
{
	if (IsRcs() && settings.rcs && settings.rcs_fov_enabled) 
		return settings.rcs_fov;
...
C_BasePlayer* LegitBot::GetClosestPlayer(CUserCmd* cmd, int& bestBone) 
{
	...
	if (kill_delay) 
	{
		if (kill_delay_time <= (int)GetTickCount64()) 
			kill_delay = false;
		else 
			return NULL;
	}
	...
	for (int i = 1; i < g_EngineClient->GetMaxClients(); ++i) 
	{
		...
		for (int bone = fromBone; bone <= toBone; bone++) 
		{
			...
			if (!g_LocalPlayer->CanSeePlayer(player, eVecTarget)) 
			{
				...
				damage = Autowall::Get().GetDamage(eVecTarget);
			...
			bestBoneFov = fov;
			if ((settings.priority == 0 && bestFov > fov) || (settings.priority == 1 && bestHealth > health) 
	...
	if (!(settings.flash_check && g_LocalPlayer->IsFlashed())) 
	{
		...
		if (GetClosestPlayer(pCmd, bestBone)) 
		{
			...
			if (settings.autostop) 
			{
				...
				if (speed > ((g_LocalPlayer->m_bIsScoped() ? weapon_data->flSpread : weapon_data->flSpreadAlt) * 0.17f)) 
				{
