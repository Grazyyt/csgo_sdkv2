    ...
    for(auto i = 0; i < g_GlowObjManager->m_GlowObjectDefinitions.Count(); i++) {
        ...
        switch(class_id) {
            case ClassId_CCSPlayer:
            {
                ...
                if(entity->HasC4() && is_enemy && g_Configurations.glow_c4_carrier) {
                    color = (Color)g_Configurations.color_glow_c4_carrier;
                    break;
                }

                if(!g_Configurations.glow_players || !entity->IsAlive())
                    continue;

                if(!is_enemy && g_Configurations.glow_enemies_only)
                    continue;
            ...
            case ClassId_CChicken:
                if(!g_Configurations.glow_chickens)
                    continue;
                entity->m_bShouldGlow() = true;
                color = (Color)g_Configurations.color_glow_chickens;
                break;
            case ClassId_CBaseAnimating:
                if(!g_Configurations.glow_defuse_kits)
                    continue;
                color = (Color)g_Configurations.color_glow_defuse;
                break;
            case ClassId_CPlantedC4:
                if(!g_Configurations.glow_planted_c4)
                    continue;
                color = (Color)g_Configurations.color_glow_planted_c4;
                break;
            default:
            {
                if(entity->IsWeapon()) {
                    if(!g_Configurations.glow_weapons)
                        continue;
                    color = (Color)g_Configurations.color_glow_weapons;
                }
            }
        }
