...
void Backtrack::OnMove(CUserCmd* pCmd)
{
	...
	LegitBot::Get().settings = g_Configurations.legitbot_items[pWeapon->m_Item().m_iItemDefinitionIndex()];

	if (!LegitBot::Get().settings.enabled)
	{
		data.clear();
		return;
	}
	...
	for (int i = 1; i <= g_GlobalVars->maxClients; ++i)
	{
		...
		if (!LegitBot::Get().settings.deathmatch && player->m_iTeamNum() == g_LocalPlayer->m_iTeamNum())
		{
			if (data.count(i) > 0) data.erase(i);
			continue;
		}
	...
	for (auto& node : data)
	{
		...
		for (auto& bd : cur_data) 
		{
			...
			if (std::fabsf(deltaTime) > LegitBot::Get().settings.backtrack_time) 
				continue;

			Math::VectorAngles(bd.hitboxPos - localEyePos, angles);
			Math::FixAngles(angles);
			float fov = LegitBot::Get().GetFovToPlayer(pCmd->viewangles, angles);
			if (best_fov > fov) 
			{
				best_fov = fov;
